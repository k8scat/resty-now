...
local corp_id = ngx.var.now_corp_id
local agent_id = ngx.var.now_agent_id
local secret = ngx.var.now_secret
local callback_scheme = ngx.var.now_callback_scheme or scheme
...
local token_expires = ngx.var.now_token_expires or "7200"
...
local function skip_oauth()
    local request_uri = ngx.var.request_uri
    local is_api = ngx.re.match(request_uri, "^/[^/]+/[^/]+/api/.*$")
    ngx.log(ngx.DEBUG, request_uri .. "is api? " .. tostring(is_api))
    if is_api then
        return true
    end
...
handle_logout()
if (not is_authorized()) and (not skip_oauth()) then
    authorize()
end
